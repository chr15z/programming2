...
public class MultiLayerRasterRGBA implements Layered //TODO: activate clause.
{
    ...
    public ArrayList<RasterizedRGB> asList() {

        // TODO: implement method.
        ArrayList<RasterizedRGB> result = new ArrayList<>();
        for (RasterizedRGB r: this) {
            result.add(r);
        }
        return result;
    }
    ...
    public RasterizedRGBIterator reversedIterator() {
        MyLayeredRevIter iter = new MyLayeredRevIter();
        buildIter(iter);
        return iter;
    }

    private void buildIter(MyLayeredRevIter iter) {
        Layered next = this;
        while(next != null) {
            new MyLayeredRevIter(iter, next);
            next = next.getBackground();
        }
    }
}
