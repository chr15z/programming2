...
public class SafeLoadSequenceFactory implements SafeFactory {
    ...
    public SafeOperation create(Scanner sc) throws FactoryException {
        ...
        SafeOperation op = SafeDoNothing.DO_NOTHING;
        ...
        try {
            Reader r = new FileReader(sc.next());
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (first && !command.equals("create")) {
                    throw new FactoryException("Invalid file format!");
                }
                first = false;
                if (!commandMap.containsKey(command)) {
                    throw new FactoryException("Invalid file format!");
                }
                op = commandMap.get(command).create(sc).after(op);
            }
            in.close();
        } catch (IOException ex) {
            throw new FactoryException("File not found!");
        }
